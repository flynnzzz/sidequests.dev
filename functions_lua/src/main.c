 ...
 */
#include </data/data/com.termux/files/usr/include/lua5.4/lua.h>
#include </data/data/com.termux/files/usr/include/lua5.4/lauxlib.h>
#include </data/data/com.termux/files/usr/include/lua5.4/lualib.h>
#include <stdlib.h>
#include <math.h>
#include <dirent.h>
...
// Posted by plinth, modified by community. See post 'Timeline' for change history
...
{
	const double base = luaL_checknumber(L, 1),
	       exp = luaL_checknumber(L, 2);
...
static void print_functions() 
{
	for (int i = 0; i < funcs_size; i++)
		printf("%d. %s id=%d nparams=%d\n", i, lua_funcs[i].name, lua_funcs[i].ref, lua_funcs[i].nparams);
}
